(function () {
  ...
  function featureToMapFeature(event) {
    ...
    return {
      ...
      properties: {
        ...
        colour: event.subtype === 'flood' ? '#38bdf8' : '#22c55e'
      }
    };
  }
      ...
      <div class="map-card">
        <div class="body">
          ...
          <div class="control-strip">
            ...
            <a class="button" href="../events/?type=natural_hazard&subtype=flood">Explorer</a>
          </div>
        </div>
      </div>
    `;
  ...
  function buildLegend() {
    els.legendList.innerHTML = `
      <div class="legend-item"><span class="swatch" style="background:#38bdf8"></span>Flood and severe-weather reports</div>
      <div class="legend-item"><span class="swatch" style="background:#22c55e"></span>Other active hazard records</div>
  ...
  async function loadHazards() {
    ...
    try {
      ...
      const selectedSources = registry.filter((source) => ['nws-active-alerts', 'nasa-eonet-floods', 'nasa-eonet-storms'].includes(source.id));
      ...
      state.features = (catalog.events || [])
        .filter((event) => ['flood', 'storm', 'cyclone', 'heatwave', 'drought'].includes(String(event.subtype || '').toLowerCase()) || String(event.source_id || '').includes('alerts'))
      ...
      setLayerData();
      updateStats();
      els.statusText.textContent = 'Could not load live flood/storm sources right now.';
    }
  }
